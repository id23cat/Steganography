...
void StegoArray::SetMessage(BYTE *mes, size_t len) throw(Exception)
{
	unsigned long crc;
	...
	//calculate CRC16 code for message
	{
		...
		crc = crc32( crc, mes, len );
		//printf("Calculated CRC: %0X\n", crc );
	}
	AddArray((BYTE*)&crc, CRC_LEN);
...
//		crc16.CleanupCRC16System();
		...
		crc2 = crc32( crc2, array+BEG_LEN+LEN_LEN, messageLength );
